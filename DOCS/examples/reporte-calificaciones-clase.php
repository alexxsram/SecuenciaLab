...
try {
  ...
  center;">Reporte de calificaciones de clase</div>', $tc=array(0,0,0),
  $lc=array(0,0,0));
  ...
  if($numRowClase != 0) {
    ...
    <tr>
    <th colspan="12" style="text-align: center; background-color: #1C6EA4;
    color: #FFFFFF;"> <b> Relación de calificaciones </b> </th>
    </tr>
    </tbody>
    </table>';
    ...
    $thead = '<thead><tr>';
    $thead .= '<th style="text-align: center; background-color: #555555;
    color: #FFFFFF;"> <b> Nombre alumno </b> </th>';
    foreach($practicas as $practica) {
      $promediosPracticas['\'' . $practica->idPractica .'\''] = 0;
      $NumeroDeAlumnosEntregadosPracticas['\'' . $practica->idPractica .'\''] = 0;
      $numeroDePracticas++;
      $thead .= '<th style="text-align: center; background-color: #555555;
      color: #FFFFFF;"> <b> ' . $practica->nombre . ' </b> </th>';
    }
    $thead .= '<th style="text-align: center; background-color: #555555;
    color: #FFFFFF;"> <b> Promedio </b> </th>';
    $thead .= '</tr></thead>';
    ...
    $sql = 'SELECT AU.codigoAlumno as codigo, CONCAT(AU.apellidoPaterno, " ",
      ...
      foreach($alumnos as $alumno) {
        $numeroDeAlumnos++;
        $sql = 'SELECT EV.*, CU.* FROM cuestionario CU INNER JOIN evaluacion EV
        ON EV.Cuestionario_idCuestionario = CU.idCuestionario WHERE
        CU.AlumnoUsuario_codigoAlumno = :auca';
        $resultado = $baseDatos->prepare($sql);
        ...
        if($numRowCalificadas > 0) {
          $tbody .= '<tr>';
          $tbody .= '<td style="text-align: center; background-color: #555555;
          color: #FFFFFF;"> ' . $alumno->codigo . ' - ' . $alumno->nombre . ' </td>';
          ...
          foreach($practicas as $practica) {
            ...
            foreach ($calificadas as $calificado) {
              if($calificado->Practica_idPractica == $practica->idPractica) {
                $promediosPracticas['\'' . $practica->idPractica .'\''] =
                $promediosPracticas['\'' . $practica->idPractica .'\''] +
                $calificado->califiacion;
                $NumeroDeAlumnosEntregadosPracticas['\'' . $practica->idPractica .'\''] =
                $NumeroDeAlumnosEntregadosPracticas['\'' . $practica->idPractica .'\''] + 1;
                $numeroDeCalificaciones++;
                $promedio = $promedio + $calificado->califiacion;
                $tbody .= '<td style="text-align: center; background-color: #D0E4F5;"> '
                . $calificado->califiacion . ' </td>';
            ...
            if(!$PracticaYaCalificada) {
              ...
              background-color: #D0E4F5;"> Sin calificación </td>';
            }
          }
          ...
          $tbody .= '<td style="text-align: center;
          background-color: #D0E4F5;"> <b>' . $promedio . '</b> </td>';
          $tbody .= '</tr>';
      ...
      $tbody .= '<tr>';
      $tbody .= '<td style="text-align: center;
      background-color: #555555; color: #FFFFFF;"> <b> Totales </b> </td>';
      foreach($practicas as $practica) {
        if(($numAlumUnaPractica =
        $NumeroDeAlumnosEntregadosPracticas['\'' . $practica->idPractica .'\'']) != 0) {
          $promedioUnaPractica =
          $promediosPracticas['\'' . $practica->idPractica .'\''] /
          $NumeroDeAlumnosEntregadosPracticas['\'' . $practica->idPractica .'\''];
        ...
        $tbody .= '<td style="text-align: center;
        background-color: #D0E4F5;"> <b> '. $promedioUnaPractica . ' </b> </td>';
      }
      $tbody .= '<td style="text-align: center;
      background-color: #D0E4F5;"> <b> '. $promedioTotalAlumnos . ' </b></td>';
      $tbody .= '</tr>';
      $tbody .= '</tbody>';
