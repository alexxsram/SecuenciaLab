...
try {
  ...
  $data = $baseDatos->query("SELECT *
    FROM alumnousuario
    WHERE codigoAlumno IN
    (SELECT AlumnoUsuario_codigoAlumno
      FROM clase_has_alumnousuario WHERE
      Clase_claveAcceso = '$claveAcceso')
      ORDER BY apellidoPaterno ASC, apellidoMaterno ASC, nombrePila ASC;")->fetchAll();
      ...
      foreach ($data as $row) {
        ...
        (SELECT idCuestionario
          ...
          (SELECT Practica_idPractica
            FROM practica
            WHERE Clase_claveAcceso = '$claveAcceso')
            AND AlumnoUsuario_codigoAlumno = '$codAlumno')")->fetchAll();
            foreach ($data as $row) {
            ...
            onclick=\"redireccionarPagina('panel-info-alumno.php?claveAccesoClase=$claveAccesoBase64
            &claveUsuario=$codAlumnoBase64');\">
            Ver gráficas <i class=\"fas fa-chart-bar\"></i> </button>

            <button type=\"button\"
            class=\"btn btn-sm btn-outline-primary\"
            onclick=\"cargarContenido('utileria/practica/',
            'calificar-entrega.php',
            'criterioCalificar=$codAlumnoBase64');\">
            ...
            if($promedioAlumno==0){
              ...
              align-items-center\">
              {$apellidoPaterno} {$apellidoMaterno} {$nombrePila} - {$codAlumno}
              ...
              Error en calculo: $promedioAlumno</span>
              $botones</li>";
            }
          }
