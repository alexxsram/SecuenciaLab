...
try {
  if($codigoAlumno != '' && $idPractica != '') {
    ...
    if($numRow != 0) {
      ...
      $sql = "SELECT * FROM evaluacion WHERE evaluacion.Cuestionario_idCuestionario = :cuesidcues";
      $resultado = $baseDatos->prepare($sql);
      $array = array(':cuesidcues'=>$cuestionario->idCuestionario);
      $resultado->execute($array);
          ...
          <form id="formCalificarPractica" name="formCalificarPractica" method="POST">
            <div class="alert alert-light text-justify" role="alert">
              <div class="form-group">
                ...
                <?php
                if($numRowEvaluacion==0){
                  echo "<input type=\"text\" class=\"form-control\" id=\"calificacion\" name=\"calificacion\" placeholder=\"0-100\" min=\"0\" max=\"100\" required=\"required\">";
                }else{
                  echo "<input type=\"text\" class=\"form-control\" id=\"calificacion\" name=\"calificacion\" value=\"$evaluacion->califiacion\" placeholder=\"0-100\" min=\"0\" max=\"100\" required=\"required\">";
                }
                ?>
              </div>
              <div class="form-group">
