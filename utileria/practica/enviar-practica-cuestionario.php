    ...
    if($uploadOk == 0) {
      ...
      if(move_uploaded_file($tmpArchivo, $target_file)) {
        if($flagPrimeraVezCuestionario){
          ...
          $resultado = $baseDatos->prepare($sql);
          $array = array(':rp1'=>$respuestaPregunta1, ':rp2'=>$respuestaPregunta2, ':rp3'=>$respuestaPregunta3, ':c'=>$conclusion, ':fe'=>$fechaEntrega, ':ra'=>$target_file, ':pip'=>$idPractica, ':auca'=>$codigoAlumno, ':nc'=>$nombreArchivoClave, ':no'=>$nombreArchivo);
          $resultado->execute($array);
          echo 'success';
        }else{
          $sql = 'UPDATE cuestionario SET respuestaPregunta1 = :rp1, respuestaPregunta2 = :rp2, respuestaPregunta3 = :rp3, conclusion = :c, fechaEntrega = :fe, rutaArchivo = :ra, Practica_idPractica = :pip, AlumnoUsuario_codigoAlumno = :auca, nombreClave = :nc, nombreOriginal = :no WHERE idCuestionario = :idCues';
          $resultado = $baseDatos->prepare($sql);
          $array = array(':rp1'=>$respuestaPregunta1, ':rp2'=>$respuestaPregunta2, ':rp3'=>$respuestaPregunta3, ':c'=>$conclusion, ':fe'=>$fechaEntrega, ':ra'=>$target_file, ':pip'=>$idPractica, ':auca'=>$codigoAlumno, ':nc'=>$nombreArchivoClave, ':no'=>$nombreArchivo, 'idCues'=>$cuestionarioAnterior->idCuestionario);
          $resultado->execute($array);
          echo 'success';
        }
      } else {
