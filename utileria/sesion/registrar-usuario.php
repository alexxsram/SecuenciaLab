...
try {
  ...
  if(is_numeric($claveUsuario)) { //Validar codigo profesor y alumno
    ...
    FROM (SELECT email
      ...
      UNION
      SELECT email
      FROM profesorusuario) as unionEmail
      WHERE email = :email';
      ...
      if($numRow == 0) {
        if($aux == 'P' || $aux == 'p') { //Comprobar profesor
          ...
          $numRow = $resultado->rowCount();
          if($numRow == 0) {
            $sql = 'INSERT INTO profesorusuario
            (codigoProfesor, nombrePila, apellidoPaterno, apellidoMaterno,
              email, PreguntaSeguridad_idPreguntaSeguridad, respuestaSeguridad,
              password)
              VALUES (:cp, :np, :ap, :am, :e, :psidps, :rs, :p)';

              $resultado = $baseDatos->prepare($sql);
              $array = array(':cp'=>$aux.$claveUsuario,
            ...
            $numRow = $resultado->rowCount();
            if($numRow == 0) {
              $sql = 'INSERT INTO alumnousuario
              (codigoAlumno, nombrePila, apellidoPaterno, apellidoMaterno,
                email, PreguntaSeguridad_idPreguntaSeguridad, respuestaSeguridad,
                password)
                VALUES (:cp, :np, :ap, :am, :e, :psidps, :rs, :p)';
